...
internal class HoverGuide
{
    private const float _paddingHeight = 24;

    private HoverGuideItem Item1 { get; }
    private HoverGuidePadding Padding1 { get; }
    private HoverGuideItem Item2 { get; }
    ...
    public void LockCard(Card card, HoverGuideTargetModifier? modifier)
    {
        LockedCard.SetTarget(card);
    ...
    private HoverGuideTarget CreateLockedTarget(Card card)
    {
        return new HoverGuideTarget(card.GetHoverText(), card.GetHoverText2(), card, LockedModifier);
    }

    public void Show(HoverGuideTarget? target1, HoverGuideTarget? target2)
    {
        if (!IsWidgetActive())
        {
            return;
    ...
    private void ShowInternal(HoverGuideTarget? target1, HoverGuideTarget? target2, bool isLocked)
    {
        if (!IsVisible())
        {
